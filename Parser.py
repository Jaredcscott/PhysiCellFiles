...
class Mesh:
    """
    Mesh: the diminsionalty of the environment
    ...
    variables: the position of each variable in the voxels
...
class Cells:
    ...
    variables: a mapping of cell attribute names to array index
    """
    ...
    def __init__(self, outputPath: str, fileFinder: Callable[[int], str] = lambda frameCount: f"output{str(frameCount).rjust(8, '0')}.xml", startingFrame: int = 0):
        ...
        for frame in itertools.count(start=int(startingFrame)):
            if not path.isfile(outputPath + fileFinder(frame)):
                self._endingFrame = frame # the ending frame number (non-inclusive)
                break
...
def Parser(outputPath: str):
    """
    Generates a parser for the given simulation output path
    """
